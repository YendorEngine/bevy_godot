...
impl InputEvent {
    pub fn get<T: SubClass<GodotInputEvent>>(&self) -> TRef<T> { self.try_get().unwrap() }
...
fn write_input_events(
    ...
    let (normal, unhandled) =
        events.0.try_iter().partition::<Vec<_>, _>(|(ty, _)| *ty == InputEventType::Normal);

    normal_evts.send_batch(normal.into_iter().map(|(_, evt)| InputEvent(evt)));
    unhandled_evts.send_batch(unhandled.into_iter().map(|(_, evt)| UnhandledInputEvent(evt)));
}
